 ...
 * Returns:
 *     errno - if cannot be migrated/isolated
		...
		struct list_head *pagelist, bool migrate_all)
{
	...
	if (PageHuge(page)) {
		if (!PageHead(page))
			return -EACCES;
		if (!isolate_hugetlb(page_folio(page), pagelist))
			return -EBUSY;
	} else {
		struct page *head;
 ...
 * Returns:
 *     errno - if the page cannot be found/isolated/migrated
 *     0 - when it doesn't have to be migrated because it is already on the
 *         target node
 *     1 - when it has been queued
 */
 ...
 * nodemask cannot be empty in this case (it would imply a task with
 * no allowed nodes).
 *
				  ...
				  int __user *status, int flags)
{
	...
	if (nodes)
		return do_pages_move(NULL, dummy_nodes, nr_pages, pages,
				     nodes, status, flags);
	else
